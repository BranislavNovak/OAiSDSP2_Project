	...
	for (int x = 0; x < xSize; x++) { 
		for (int y = 0; y < ySize; y++) {
			...
			if (newY >= 0 && newY < ySize && x >= 0 && y < xSize) {
				output[3 * x + y * xSize * 3] = input[3 * x + newY * xSize * 3];
				output[3 * x + 1 + y * xSize * 3] = input[3 * x + 1 + newY * xSize * 3];
				output[3 * x + 2 + y * xSize * 3] = input[3 * x + 2 + newY * xSize * 3];
			}
			else {
				output[3 * x + y * xSize * 3] = 0;
				output[3 * x + 1 + y * xSize * 3] = 0;
				output[3 * x + 2 + y * xSize * 3] = 0;
			}
		}
	}
}
